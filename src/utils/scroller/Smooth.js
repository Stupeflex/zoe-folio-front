...
export default class SmoothScroll extends Core {
  constructor(options = {}) {
  ...
  setScrollLimit() {
    ...
    if (this.direction === 'vertical') {
      ...
      let nodes = this.el.children;
      for (let i = 0; i < nodes.length; i++) {
        totalHeight += nodes[i].offsetHeight;
      }
      this.instance.limit.y = totalHeight - this.windowHeight;
    }
  }
  ...
  moveScrollBar(e) {
    if (this.isDraggingScrollbar) {
      requestAnimationFrame(() => {
        let x =
          ((((e.clientX - this.scrollbarBCR.left) * 100) /
            this.scrollbarWidth) *
            this.instance.limit.x) /
          100;
        let y =
          ((((e.clientY - this.scrollbarBCR.top) * 100) /
  ...
  addElements() {
    ...
    els.forEach((el, index) => {
      ...
      if (section === null) {
        top =
          targetElBCR.top + this.instance.scroll.y - getTranslate(targetEl).y;
        left =
          targetElBCR.left + this.instance.scroll.x - getTranslate(targetEl).x;
      } else {
        if (!section.inView) {
          top =
            targetElBCR.top -
  ...
  scrollTo(target, options = {}) {
    ...
    const duration = !isNaN(parseInt(options.duration))
    ...
    if (typeof target !== 'number') {
      ...
      if (parentSection) {
        ...
        console.warn('no parent section');
