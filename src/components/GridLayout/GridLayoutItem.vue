...
const previewStyle = computed(() => {
  const transform = formatCssTranslate({
    ...
    x: `calc(${cssCellWidth(-1, true)} - ${cssUnit(2)})`,
  });
...
const style = computed(() => {
  const s = doPreview.value
    ? previewStyle.value
    : formatItemStyle(dimensions.value);
  return {
    ...s,
    cursor: cursor.value,
  };
...
const onMouseMove = (pos: Vector2) => {
  if (mouseDown.value) {
    ...
    if (props.editable && props.movable) {
      ...
      movedDistance.x = gridDistance.x;
      movedDistance.y = gridDistance.y;
      ...
      if (b.length === 0) {
        emit('move', props.item.id, gridDelta);
      }
      if (b.length === 1) {
        emit('resize', props.item.id, {
          [b[0]]: gridDelta[b[0] as Border],
        });
      }
      if (b.length === 2) {
        b.forEach((side) => {
          emit('resize', props.item.id, {
            [side]: gridDelta[side as Border],
          });
        });
      }
    }
...
//
// watch(mouseDragDistance, (dist) => {
//   if (props.editable && props.movable) {
//     const distance = screenToGrid(dist);
//     movedDistance.x = distance.x;
//     movedDistance.y = distance.y;
//   } else {
//     console.log(dist);
//   }
// });
//
// watch(movedDistance, (dist) => {
//   const b = Object.entries(activeBorders)
//     .filter(([_, v]) => v)
//     .map(([s]) => s);
//   const delta = {
//     x: preMovePosition.x + dist.x,
//     y: preMovePosition.y + dist.y,
//     right: preMoveSize.width + dist.x,
//     bottom: preMoveSize.height + dist.y,
//     top: preMoveSize.height - dist.y,
//     left: preMoveSize.width - dist.x,
//   };
//   console.log(dist);
//   if (b.length === 0) {
//     emit('move', props.item.id, delta);
//   }
//   if (b.length === 1) {
//     emit('resize', props.item.id, {
//       [b[0]]: delta[b[0] as Border],
//     });
//   }
//   if (b.length === 2) {
//     b.forEach((side) => {
//       emit('resize', props.item.id, {
//         [side]: delta[side as Border],
//       });
//     });
//   }
// });
